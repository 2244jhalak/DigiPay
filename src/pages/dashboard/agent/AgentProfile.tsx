  ...
  return (
    <Card className="w-full max-w-xl mx-auto mt-10 shadow-2xl rounded-3xl bg-gradient-to-tr from-white/30 to-white/10 backdrop-blur-lg border border-white/20">
      <CardHeader className="flex flex-col items-center gap-4 p-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-300">{user.role.charAt(0).toUpperCase() + user.role.slice(1)}</p>
        
        {user.isApproved && (
        <p
          className={`text-sm text-white dark:text-white p-2 rounded-md ${
            user.isApproved === "approve" ? "bg-green-500" : "bg-red-500"
          }`}
        >
          {user.isApproved.charAt(0).toUpperCase() + user.isApproved.slice(1)}
        </p>
        )}
