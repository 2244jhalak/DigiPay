...
import { useGetWalletQuery, useCashInMutation } from "@/redux/walletSlice";
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    if (!amount || amount <= 0) {
      Swal.fire({
        ...
        text: "Please enter a valid amount to cash in.",
        confirmButtonColor: "#FF6B35",
      });
      return;
    }
    ...
    try {
      const result = await cashIn({ amount, toAuthId: receiverId }).unwrap();
      console.log(result)

      Swal.fire({
        icon: "success",
        title: "Cash In!",
        text: `You cash in ৳${amount} to user ${receiverId}. Your new balance: ৳${result.agentBalance}. Your Commission: ৳${result.commission}`,
  ...
  return (
    <Card className="w-full max-w-lg mx-auto mt-10 shadow-2xl rounded-3xl bg-gradient-to-tr from-white/30 to-white/10 backdrop-blur-lg border border-white/20">
      <CardHeader className="flex flex-col items-center gap-4 p-6">
        <CardTitle className="text-2xl font-bold text-gray-800 dark:text-white">Cash In</CardTitle>
