...
import { useGetWalletQuery, useSendMoneyMutation } from "@/redux/walletSlice";
...
export default function SendMoney() {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const result = await sendMoney({ amount, toAuthId: receiverId }).unwrap();

      Swal.fire({
        icon: "success",
        title: "Money Sent!",
        text: `You sent ৳${amount} to user ${receiverId}. Your new balance: ৳${result.senderBalance}`,
  ...
  return (
    <Card className="w-full max-w-lg mx-auto mt-10 shadow-2xl rounded-3xl bg-gradient-to-tr from-white/30 to-white/10 backdrop-blur-lg border border-white/20">
      <CardHeader className="flex flex-col items-center gap-4 p-6">
        <CardTitle className="text-2xl font-bold text-gray-800 dark:text-white">Send Money</CardTitle>
