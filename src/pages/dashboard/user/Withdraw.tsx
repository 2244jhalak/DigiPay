...
import { useGetWalletQuery, useWithdrawMutation } from "@/redux/walletSlice";
...
export default function Withdraw() {
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    if (walletData && amount > walletData.wallet.balance) {
      Swal.fire({
        icon: "error",
    ...
    try {
      const result = await withdraw(amount).unwrap();

      Swal.fire({
        icon: "success",
        title: "Withdrawal Successful",
        text: `Your wallet balance is now ৳${result.balance}`,
  ...
  return (
    <Card className="w-full max-w-lg mx-auto mt-10 shadow-2xl rounded-3xl bg-gradient-to-tr from-white/30 to-white/10 backdrop-blur-lg border border-white/20">
      <CardHeader className="flex flex-col items-center gap-4 p-6">
        <CardTitle className="text-2xl font-bold text-gray-800 dark:text-white">Withdraw Wallet</CardTitle>
        ...
        <form onSubmit={handleWithdraw} className="space-y-4">
          ...
          <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
            <Button
              ...
              className="relative w-full bg-gradient-to-r from-[#FF6B35] to-[#FF3B30] hover:from-[#FF3B30] hover:to-[#FF6B35] text-white font-semibold py-3 rounded-xl shadow-lg transition-all duration-300 flex items-center justify-center"
            >
              {withdrawLoading ? (
                <svg
